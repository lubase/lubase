...
public class ServerMacroService {
    ...
    @Autowired
    ServerMacroExtendAdapter serverMacroExtendAdapter;
    ...
    public String getServerMacroByKey(String macroKey) {
        ...
        if (macroKey.equals("@@S.date")) {
            ...
            ServerMacroExtend macroExtend = serverMacroExtendAdapter.getServerMacroExtendByName(macroKey);
            if (macroExtend != null) {
                LoginUser user = appHolderService.getUser();
                Long appId = 0L;
                String userId = "";
                if (user != null) {
                    appId = user.getSessionAppId();
                    userId = user.getId().toString();
                }
                val = macroExtend.getValue(appId, userId);
            } else {
